...
class ONNXEmbeddingFunction(EmbeddingFunction[Documents]):
    """
    Base class for embedding functions using ONNX runtime.

    This class handles downloading models from Hugging Face (or mirrors),
    tokenization, and inference using ONNX Runtime.
    """
    ...
    def __init__(
        ...
        # Update DOWNLOAD_PATH based on the model name if it's different from default
    ...
    def _download_from_huggingface(self) -> bool:  # noqa: C901
        ...
        try:
            ...
            for hf_filename, local_filename in files_to_download.items():
                ...
                # For generic ONNX models, the HF_MODEL_ID might need to change if model_name changes.
                # Here we assume HF_MODEL_ID corresponds to the model we want.
                # If this class is used for other models, HF_MODEL_ID should probably be updated or passed in.
                # But for this refactor, we keep it simple.
    ...
    def name() -> str:
        """
        Return a unique name identifier for routing.
        This method should be overridden by subclasses or instances if they want to be registered.
        """
        return "onnx"
    ...
    def build_from_config(config: dict[str, Any]) -> "ONNXEmbeddingFunction":
        """
        Restore an instance from config.
        """
...
class DefaultEmbeddingFunction(ONNXEmbeddingFunction):
    ...
    def __init__(
        ...
        super().__init__(model_name=model_name, preferred_providers=preferred_providers)
...
class EmbeddingFunctionRegistry:
    ...
    def _initialize(cls) -> None:
        ...
        cls._registry["onnx"] = ONNXEmbeddingFunction
